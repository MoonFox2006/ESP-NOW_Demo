...
bool EspNowGeneric::begin() {
  if (esp_now_init() != ESPNOW_OK)
    return false;
  if (esp_now_set_self_role(ESP_NOW_ROLE_COMBO) != ESPNOW_OK) {
    esp_now_deinit();
    return false;
  }
...
bool EspNowGeneric::addPeer(const uint8_t *mac) {
  return (esp_now_add_peer((uint8_t*)mac, ESP_NOW_ROLE_COMBO, _channel, NULL, 0) == ESPNOW_OK);
}

bool EspNowGeneric::addPeerSecure(const uint8_t *mac, const uint8_t *key, uint8_t keylen) {
  return (esp_now_add_peer((uint8_t*)mac, ESP_NOW_ROLE_COMBO, _channel, (uint8_t*)key, keylen) == ESPNOW_OK);
}
...
bool EspNowGeneric::send(const uint8_t *mac, const uint8_t *data, uint8_t len) {
  return (esp_now_send((uint8_t*)mac, (uint8_t*)data, len) == ESPNOW_OK);
}

bool EspNowGeneric::sendAll(const uint8_t *data, uint8_t len) {
  return (esp_now_send(NULL, (uint8_t*)data, len) == ESPNOW_OK);
}
...
bool EspNowGeneric::sendBroadcast(const uint8_t *data, uint8_t len) {
  ...
  return (esp_now_send(mac, (uint8_t*)data, len) == ESPNOW_OK);
}
...
int8_t espNowFindServer(uint8_t *mac) {
  int8_t result;
  char ssid[sizeof(ESPNOW_SERVER_AP)];
